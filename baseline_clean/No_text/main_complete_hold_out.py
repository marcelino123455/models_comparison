...
def save_best_to_csv(best_dict, filename="best_f1_by_embedding.csv"):
    rows = []
...
def main():
    ...
    # D = ['emotion', 'Time signature', 'Artist(s)', 'song']
    ...
    print("Applying Hold Out with tf-idf embbedings only")
    ...
    for column  in  total_array:
        ...
        X, y = load_TFIDF_embbedings(csv_path, sample_size=_SAMPLE_SIZE, columns_tf_idfizable = TF_idezeable,  max_features = MAX_FEATURES, steaming= STEAMING, remove_stopwords = REMOVESTW)   
        ...
        output_dir = f"outputs_hold_out/0/{column}"
        if MLP_: 
            nets = {
                # Pequeña y rápida
    ...
    df_best = save_best_to_csv(best_by_emb)
