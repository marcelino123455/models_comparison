...
def save_best_to_csv(best_dict, filename="best_f1_by_embedding.csv", list_use_stop = []):
    ...
    for emb_type, info in best_dict.items():
        row = {
            ...
            "use_stop_words": True if list_use_stop[i] == None else False
        }
        i+=1
...
def main():
    ...
    print("Trainning one by one vector")
    len_COL_TF_IDF = len(COL_TF_IDF)
    len_N_cols = len(N_cols)
    total_Columns = len_COL_TF_IDF+ len_N_cols
    print(f"You are using {total_Columns} columns for TF-IDF")
    ...
    for column  in  total_array:
        print(f"\n{'#' * 50}")
        print(f"Running experiment without {column.upper()} embeddings")
        X, y = None, None
        ...
        #     TF_idezeable = COL_TF_IDF
            ...
            X, y,column_without_stp = load_TFIDF_embbedings(csv_path, sample_size=_SAMPLE_SIZE, columns_tf_idfizable = TF_idezeable,  max_features = MAX_FEATURES, steaming= STEAMING, remove_stopwords = REMOVESTW)   
            # if column_stp is None: 
            colums_witouth_stop_words.append(column_without_stp)
        ...
        if NUMERICCOlS: # !Numeric cols only works ok with all dataset since yes skdjsk
            ...
            if column in N_cols:
                colums_witouth_stop_words.append(None)
            ...
            #     The_numeric_cols = N_cols
                ...
                indices_validos = [i for i in indices_to_remove if i < len(df)]
                df = df.drop(indices_validos).reset_index(drop=True)

                if column == 'Loudness (db)':
                ...
                if X is None:
                    X = df[The_numeric_cols].to_numpy()
                    # y = df["label"].to_numpy()
                    df['Explicit_binary'] = (df['Explicit'].str.lower() == 'yes').astype(int)
                    y = df['Explicit_binary']
                    X, y = elimnate_index(X, y)
                    ...
                    X = np.concatenate([X, df[The_numeric_cols].to_numpy()], axis=1)
        ...
        output_dir = f"outputs_only_one/0/{column}"
        if MLP_: 
            nets = {
                # Pequeña y rápida
    ...
    df_best = save_best_to_csv(best_by_emb, "f1_score_by_cols.csv", colums_witouth_stop_words )
