...
def main():
    ...
    for embedding_type in embb_types:
        ...
        X_train = np.concatenate([X_train_text, X_train_Numeric, X_train_cat], axis=1)
        X_test  = np.concatenate([X_test_text,  X_test_Numeric, X_test_cat],  axis=1)
        ...
        output_dir = f"outputs_text_plus_numerical_categorical{out}/2/{embedding_type}"
        if SCALED:
            scaler = MinMaxScaler(feature_range=(0, 1))
