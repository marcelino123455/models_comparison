...
#     column_withouth_stopwords = None
#     # Preprocess text
...
#     # bucle try para desactivar los stop words
#     try:
#         print("Ejemplo de textos antes de TF-IDF:", df['processed_text'].head(10).tolist())
...
#             print("Empty vocab (solo stopwords). Reintentando sin eliminar stopwords...")
#             df['combined_text'] = df[columns_tf_idfizable].fillna('').agg(' '.join, axis=1)
#             # df[columns_tf_idfizable]
#             print("df con las columnas es:", df[columns_tf_idfizable].head(10))

#             df['processed_text2'] = df['combined_text'].apply(
#                 lambda x: preprocessor.preprocess(x, remove_stopwords=False, apply_stemming=steaming)
#             )
#             print("processed_text2", df['processed_text2'].head(10).tolist())

#             column_withouth_stopwords = columns_tf_idfizable[0]
#             vectorizer = TfidfVectorizer(
#                 max_features=max_features,
#                 min_df=3,
#                 max_df=0.92,
...
#             try:
#                 X_tfidf = vectorizer.fit_transform(df['processed_text2'])

#             except ValueError as e:
#                 print("TF-IDF falló de nuevo, usando One-Hot Encoding en la columna", columns_tf_idfizable[0])
#                 X_tfidf = pd.get_dummies(df[columns_tf_idfizable[0]])
#         else:

#             raise  

#     if isinstance(X_tfidf, pd.DataFrame):
#         X = X_tfidf.values  # opcional, si quieres un numpy array
#     else:
#         X = X_tfidf.toarray()  # para TF-IDF

#     # X = X_tfidf.toarray()
#     print(f"Label distribution: {df['Explicit_binary'].value_counts().to_dict()}")
#     print(f"X shape: {X.shape}")
#     print(f"y shape: {y.shape}")
#     return X, y, column_withouth_stopwords
...
def get_best_by_embedding(resultados_globales):
    best_dict = {}
    for emb_type, models in resultados_globales.items():
        # crea un DataFrame directo
        df = pd.DataFrame(models).T  # index=model_name, cols=metrics
        df = df.astype(float)
        # ordena por f1_score
        ...
        best_dict[emb_type] = {
            ...
            "all_metrics": df_sorted.iloc[0].to_dict()
        }
    return best_dict

def save_best_to_csv(best_dict, filename="best_f1_by_embedding.csv", list_use_stop = []):
    rows = []
    i = 0
    for emb_type, info in best_dict.items():
        print("Feature: ", emb_type)
        col_without_sw = False
        if emb_type in list_use_stop:
            col_without_sw = True
        row = {
            "embedding": emb_type,
            "best_model": info["best_model"],
            "best_f1_score": info["best_f1_score"],
            "use_stop_words": col_without_sw 
        }
        i+=1
        row.update(info["all_metrics"])  # añade accuracy, precision, recall...
        rows.append(row)
...
def main():
    ...
    path_outputs = "outputs_ablation_remove_one_feature/1"
    ...
    if TESTING:
        COL_TF_IDF = ['emotion', 'Time signature']
        N_cols = ['Release Date','Tempo']
        _SAMPLE_SIZE = 100
    ...
    COL_TF_IDF = CAT_ALL # + COL_TF_IDF
    print("Trainning one by one features for TF-IDF embbedings")
    print("len_COL_TF_IDF: ", len(COL_TF_IDF))
    print("len_N_cols: ", len(N_cols))
    ...
    ##  Cleaning principal ### 
    print("\n" + "="*50)
    print("Loading and preprocessing main DataFrame...")
    print("="*50)

    df_master = pd.read_csv(csv_path, nrows=_SAMPLE_SIZE)
    indices_to_remove = get_array("faltantes_according_token.json")

    if indices_to_remove[-1] > df_master.shape[0]:
        print("You are executing in testing way")
    else:
        df_master = df_master.drop(indices_to_remove).reset_index(drop=True)
    
    # Procesar 'Loudness (db)'
    if 'Loudness (db)' in df_master.columns:
        if not np.issubdtype(df_master['Loudness (db)'].dtype, np.number):
            print("Processing 'Loudness (db)'...")
            df_master['Loudness (db)'] = df_master['Loudness (db)'].astype(str).str.replace('db', '', regex=False)
            df_master['Loudness (db)'] = pd.to_numeric(df_master['Loudness (db)'], errors='coerce')
    
    # Procesar 'Length'
    if 'Length' in df_master.columns:
        if not np.issubdtype(df_master["Length"].dtype, np.number):
            print("Processing 'Length'...")
            df_master["Length"] = pd.to_timedelta("00:" + df_master["Length"]).dt.total_seconds().astype(int)
       # Procesar 'Release Date'
    if 'Release Date' in df_master.columns:
        ...
        df_master["Release Date Clean"] = df_master["Release Date"].apply(clean_date)
        df_master["Release Date Parsed"] = pd.to_datetime(
            df_master["Release Date Clean"],
            errors='coerce',
            dayfirst=True
        )
        
        # Crear nuevas columnas
        df_master["Release_Year"] = df_master["Release Date Parsed"].dt.year
        df_master["Release_Month"] = df_master["Release Date Parsed"].dt.month
        df_master["Release_Day"] = df_master["Release Date Parsed"].dt.day
        
        # Eliminar columnas intermedias
        df_master.drop(
            columns=["Release Date", "Release Date Clean", "Release Date Parsed"],
            inplace=True,
            errors='ignore'
        )
        
        # Actualizar N_cols para reflejar los cambios
        if 'Release Date' in N_cols:
            N_cols = [c for c in N_cols if c != 'Release Date']
            N_cols.extend(['Release_Year', 'Release_Month', 'Release_Day'])
        
    print(f"New numeric columns: {N_cols}")
    total_array = COL_TF_IDF + N_cols

    print(f"Total de columnas [{len(total_array)}]: ", total_array)

    colums_witouth_stop_words = []

    if TESTING:
        total_array = ['emotion', 'Time signature', 'Release Date','Tempo']

    for column  in  total_array:
        print(f"\n{'#' * 50}")
        print(f"Running experiment without {column.upper()} feature")
        X = None
        
        # For remove textual columns in TF-IDF
        if column in COL_TF_IDF:
            TF_idezeable = [col for col in COL_TF_IDF if col != column]
        else:
            TF_idezeable = COL_TF_IDF

        # if column in COL_TF_IDF:
        print(f"[{column}] in COL_TF_IDF")
        X, y_, vectorizer = load_TFIDF_embbedings(csv_path, sample_size=_SAMPLE_SIZE, columns_tf_idfizable = TF_idezeable,  max_features = MAX_FEATURES, steaming= STEAMING, remove_stopwords = REMOVESTW)   
        y = y_

        if NUMERICCOlS:
            if column in N_cols:
                The_numeric_cols = [col for col in N_cols if col != column]
            else:
                The_numeric_cols = N_cols
            
            df_NumericCols = df_master[The_numeric_cols].fillna(0)
            print(f"Numeric features shape: {df_NumericCols.shape}")

                # df[The_numeric_cols] = df[The_numeric_cols].fillna(0)
        # ---------- SPLIT GENERAL ---------
        ...
        # if column in COL_TF_IDF:
        # [Categorical and Text] splitting
        X_train_text = X[train_idx] # X = text feature
        X_test_text  = X[test_idx]
        try:
            # Intento inicial con stopwords si corresponde
            vectorizer = TfidfVectorizer(
                max_features=MAX_FEATURES,
                min_df=3,
                max_df=0.92,
            ...
            if "empty vocabulary" in str(e):
                colums_witouth_stop_words.append(column)
                print(f"[{column}] Vocabulario vacío (solo stopwords). Reintentando sin eliminar stopwords...")
                # Segundo intento sin eliminar stopwords
                vectorizer = TfidfVectorizer(
                    max_features=MAX_FEATURES,
                    min_df=3,
                    max_df=0.92,
                ...
                try:
                    ...
                    print(f"[{column}] TF-IDF volvió a fallar. Usando One-Hot Encoding como respaldo...")
                    # One-Hot Encoding de respaldo
                    # import pandas as pd
                    X_train_text = pd.get_dummies(X_train_text)
                    X_test_text = pd.get_dummies(X_test_text)
                    # Alinear columnas del train y test (importante)
                    X_train_text, X_test_text = X_train_text.align(X_test_text, join='left', axis=1, fill_value=0)
                    X_train_text = X_train_text.values
                    X_test_text = X_test_text.values
            else:
                # Si es otro error, lo repropaga
                raise
        ...
        print("X_train_Numeric: ",X_train.shape)
        print("X_train_Numeric: ", X_test.shape)
        ...
        output_dir = f"{path_outputs}/{column}"
        if MLP_: 
            nets = {
                # Pequeña y rápida
            ...
            for name, capas in nets.items():
                ...
                resultados_globales.setdefault(column, {})
                resultados_globales[column][f"MLP_{trainable_params}"] = resumencito
        ...
        resumen = train_models(X_train, X_test, y_train, y_test, dir_=output_dir, embedding_type=column)
        # resultados_globales[embedding_type] = resumen
        resultados_globales.setdefault(column, {})
        resultados_globales[column].update(resumen)
    ...
    for emb_type, models_dict in resultados_globales.items():
        ...
        for modelo, metricas in sorted(models_dict.items(), key=lambda x: x[1]["f1_score"], reverse=True):
            print(f"{modelo}: {metricas}")
    best_by_emb = get_best_by_embedding(resultados_globales)
    df_best = save_best_to_csv(best_by_emb, f"{path_outputs}/ablation_remove_one_feature.csv", colums_witouth_stop_words )
    print("\n Mejores modelos por embedding:")
    print(df_best)

    # Mejor global
    global_best = df_best.sort_values("best_f1_score", ascending=False).iloc[0]
    print(f"\n Mejor global: {global_best['embedding']} "
        f"con {global_best['best_model']} "
        f"(f1={global_best['best_f1_score']:.4f})")
