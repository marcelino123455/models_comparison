...
def main():
    ...
    path_outputs = "outputs_ablation_one_feature/0"
    ...
    # Abltaion study onli with metadata
    ...
    for column  in  total_array:
        print(f"\n{'#' * 50}")
        print(f"Running experiment with {column.upper()} feature")
        X = None
        # For remove textual columns in TF-IDF
        if column in COL_TF_IDF:
            print(f"[{column}] in COL_TF_IDF")
            TF_idezeable = [column] # Only train eith the corresponding column
        ...
        if NUMERICCOlS: 
            if column in N_cols:
                print(f"[{column}] in N_cols")
                The_numeric_cols = [column] # The same only this column
        ...
        if column in COL_TF_IDF:
            ...
            try:
                ...
                if "empty vocabulary" in str(e):
                    ...
                    try:
                        ...
                        # import pandas as pd
                        X_train = pd.get_dummies(X_train_text)
                        X_test = pd.get_dummies(X_test_text)
            ...
            # [Numeric] splitting
            X_train = df_NumericCols.iloc[train_idx].values
            X_test  = df_NumericCols.iloc[test_idx].values
    ...
    df_best = save_best_to_csv(best_by_emb, f"{path_outputs}/ablation_one_feature.csv", colums_witouth_stop_words )
