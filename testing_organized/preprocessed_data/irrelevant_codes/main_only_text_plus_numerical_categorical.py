...
def main():
    ...
    # Two types, with high and low cardinality
    ...
    print("CAT_LOW", len(CAT_LOW), CAT_LOW)
    ...
    for embedding_type in embb_types:
        X, y = None, None  
        if TEXT:
            print("Using text columns (Lyrics of the song) on the embeddings")
        ...
        if CATEGORICALCOLS: 
            ...
            # Cleaning naa
            for col in CAT_HIGH + CAT_LOW:
                df[col] = df[col].fillna("").astype(str)
            
            # --- Baja cardinalidad con OneHot ---
            ohe = OneHotEncoder(sparse_output=False, handle_unknown='ignore')
            X_low = ohe.fit_transform(df[CAT_LOW])

            # --- Alta cardinalidad con FeatureHasher ---
            # high_dicts = df[CAT_HIGH].apply(lambda row: dict(row), axis=1)
            high_dicts = df[CAT_HIGH].apply(
                lambda row: {f"{col}_{row[col]}": 1 for col in CAT_HIGH},
                axis=1
            )
            hasher = FeatureHasher(n_features=N_FEATURE_HASHER, input_type='dict')  
            X_high = hasher.transform(high_dicts).toarray()
            # X = np.concatenate([X, X_low, X_high], axis=1)
            if X is None:
                X = np.concatenate([X_low, X_high], axis=1)
            else:
                X = np.concatenate([X, X_low, X_high], axis=1)
            print("Nuevas dimensiones X con categóricas:", X.shape)
        ...
        output_dir = f"outputs_text_plus_numerical_categorical{out}/0/{embedding_type}"
        if SCALED:
            scaler = MinMaxScaler(feature_range=(0, 1))
