...
def elimnate_y_index(y, path= "faltantes_according_token.json"):
    ...
    print("E eliminarán los siguientes indices", elminar_)
    print("El último valor de eliminar es: ", elminar_[-1])
...
def train_models_cv(X_train, X_test, y_train, y_test, dir_="output", embedding_type="tfidf", n_splits=5):
    models = {
        "Logistic Regression": LogisticRegression(
            penalty='l2',
            C=1,
    ...
    for name, model in models.items():
        ...
        resumen_metricas[name] = {
            ...
            "precision_cv_mean": round(float(np.mean(precs)), 4),
            "precision_cv_std": round(float(np.std(precs)), 4),

            "recall_cv_mean": round(float(np.mean(recs)), 4),
            "recall_cv_std": round(float(np.std(recs)), 4),

            "f1_cv_mean": round(float(np.mean(f1s)), 4),
            "f1_cv_std": round(float(np.std(f1s)), 4),
        }

        print("\nPromedio CV (validación interna):")
        print(resumen_metricas[name])
        print(f"Hiperparámetros: {model.get_params()}")
        ...
        os.makedirs(dir_, exist_ok=True)
        filename = os.path.join(dir_, f"conf_matrix_test_{name.replace(' ', '_').lower()}.png")
        plt.savefig(filename)
        print(f"Confusion matrix Test saved as: {filename}")
        plt.close()
    ...
    print("\n\nResumen Final:")
    for modelo, metricas in sorted(resumen_metricas.items(), key=lambda x: x[1]["f1_score_test"], reverse=True):
        print(f"{modelo}: {metricas}")
...
def train_mlp_kfold(X_train, X_test, y_train, y_test, nets, device, output_dir, embedding_type, n_splits=5, n_epochs=50):
    resultados_globales = {}
    resumen_metricas = {}

    skf = StratifiedKFold(n_splits=n_splits, shuffle=True, random_state=42)

    # convertir a tensores (para usar en dataset)
    X_train_tensor = torch.tensor(X_train, dtype=torch.float32)
    ...
    for name, capas in nets.items():
        ...
        for fold, (train_idx, val_idx) in enumerate(skf.split(X_train, y_train)):
            ...
            # entrenar
            _, _, best_f1, best_pred, _, best_labels, _ = train_deep_rn(
                net, train_loader, val_loader, criterion, optimizer, device,
                n_epoch=n_epochs, target_f1=0.95, print_every=10
            )

            f1s.append(best_f1)
            accs.append(accuracy_score(best_labels, best_pred))
            precs.append(precision_score(best_labels, best_pred))
            recs.append(recall_score(best_labels, best_pred))
            if n_params is None:
                n_params = sum(p.numel() for p in net.parameters() if p.requires_grad)

        resumen_metricas[f"MLP_{n_params}"] = {
            "accuracy_cv_mean": round(float(np.mean(accs)), 4),
            "accuracy_cv_std": round(float(np.std(accs)), 4),
        ...
        # === Reentrenar con todo el TRAIN y evaluar en TEST ===
        train_loader_full = DataLoader(TensorDataset(X_train_tensor, y_train_tensor), batch_size=64, shuffle=True)
        test_loader       = DataLoader(TensorDataset(X_test_tensor, y_test_tensor), batch_size=64, shuffle=False)

        net_final = MLP(capas).to(device)
        criterion = nn.BCEWithLogitsLoss()
        optimizer = optim.AdamW(net_final.parameters(), lr=0.001)

        _, _, best_f1_test, best_pred, _, best_labels, _ = train_deep_rn(
            net_final, train_loader_full, test_loader, criterion, optimizer, device,
            n_epoch=n_epochs, target_f1=0.95, print_every=10
        )
        ...
        resumen_metricas[f"MLP_{n_params}"].update({
            "accuracy_test": round(acc, 4),
            "precision_test": round(prec, 4),
            "recall_test": round(rec, 4),
            "f1_score_test": round(f1, 4)
        ...
        cm = confusion_matrix(best_labels, best_pred, labels=[0, 1])
        plt.figure(figsize=(5, 4))
        sns.heatmap(
        ...
        plt.title(f'Confusion Matrix (Test) - MLP {name}')
        plt.xlabel('Predicted')
        plt.ylabel('Actual')
        os.makedirs(output_dir, exist_ok=True)
        cm_path = os.path.join(output_dir, f"cm_mlp_{name}.png")
        plt.savefig(cm_path)
        plt.close()
        print(f"Confusion matrix Test saved: {cm_path}")

        resultados_globales.setdefault(embedding_type, {})
        resultados_globales[embedding_type][f"MLP_{n_params}"] = resumen_metricas[f"MLP_{n_params}"]

    return resumen_metricas, resultados_globales
...
def elimnate_index(X, y, path= "faltantes_according_token.json"):
    ...
    # print("El ultimo valor es: ", elminar_[-1])
    print("El valor de y[0] es", y.shape[0])
    if elminar_[-1] > y.shape[0]:
        return X, y
...
def remove_cols(ignorated_cols, numerical_cols, categorical_cols ):
    cols_to_remove = []
    # new_numeric_cols = [col for col in numerical_cols if col not in ignorated_cols]
    # new_categorical_cols = [col for col in categorical_cols if col not in ignorated_cols]
...
def main():
    ...
    # This part is to establish the columns that we aren't going to use
    ignorated_cols = [
        'Similar Artist 1', 
        'Similar Artist 2', 
        'Similar Artist 3', 
    ...
    COL_TF_IDF = T # It doesn't matter; TEXT = False
    ...
    print("\nCAT_LOW", len(CAT_LOW), CAT_LOW)
    print("CAT_HIGH", len(CAT_HIGH), CAT_HIGH)
    ...
    if COL_TF_IDF == T:
        cols_type = "T"
        npy_path = f"{path_DATA}/new_embbedings_khipu/LB_fuss/lb_khipu_T.npy" 

    
    if TESTING:
        COL_TF_IDF = ['emotion']
    # Actualización por si se decide eliminar algunas columnas
    N_cols, COL_TF_IDF = remove_cols(ignorated_cols, N_cols, COL_TF_IDF)
    # No removemos alguna columna
    print("After removing some columns that ignorated, for TF-IDF embbedings you are selecteing this columns:" )
    print("-->", COL_TF_IDF)
    if NUMERICCOlS:
        print("After removing some columns that ignorated, for numeric cols you are selecteing this columns:" )
        print("-->", N_cols)
    ...
    for embedding_type in embb_types:
        if TEXT: 
            if embedding_type == 'lyrics_bert':
                ...
                X, y = elimnate_index(X, y) # Consistencia coon los embbeidngs

            else: 
                print(f"\n{'#' * 50}")
                print(f"Running experiment with {embedding_type.upper()} embeddings")
                X, y, vectorizer = load_TFIDF_embbedings(csv_path, sample_size=_SAMPLE_SIZE, columns_tf_idfizable = COL_TF_IDF,  max_features = MAX_FEATURES, steaming= STEAMING, remove_stopwords = REMOVESTW)   

        if CATEGORICALCOLS:
            if embedding_type == 'tfidf':
                print(f"Running experiment with {embedding_type.upper()} embeddings")

                print("Contaning the categorical cols")
                CAT_EMB, _, vectorizer_cat = load_TFIDF_embbedings(csv_path, sample_size=_SAMPLE_SIZE, columns_tf_idfizable = CAT_ALL,  max_features = MAX_FEATURES, steaming= STEAMING, remove_stopwords = REMOVESTW)   
                y = _
                
            elif embedding_type == 'lyrics_bert':
                print(f"Running experiment with {embedding_type.upper()} embeddings")
                
                npy_path_cat = f"{path_DATA}/new_embbedings_khipu/LB_fuss/lb_khipu_F.npy" 
                CAT_EMB, _ = load_LB_embbedings(csv_path, npy_path_cat, sample_size=_SAMPLE_SIZE)
                CAT_EMB, _ = elimnate_index(CAT_EMB, _) # Seguiremos eliminando los 13 indices 

                
            elif embedding_type == 'gpt':
                print(f"Running experiment with {embedding_type.upper()} embeddings")
                
                npy_path_gpt_cat = f"{path_DATA}/gpt_F_cat/gpt_fussioned/gpt_F.npy"
                CAT_EMB, _ = load_gpt_embbedings(csv_path, npy_path_gpt_cat, sample_size=_SAMPLE_SIZE)
                CAT_EMB, _ = elimnate_index(CAT_EMB, _) # Seguiremos eliminando los 13 indices 
        ...
        if NUMERICCOlS: # !Numeric cols only works ok with all dataset since yes skdjsk
            ...
            # Release Date
            df["Release Date Clean"] = df["Release Date"].apply(clean_date)

            df["Release Date Parsed"] = pd.to_datetime(
                df["Release Date Clean"],
                errors='coerce',
                dayfirst=True
            )
            df["Release_Year"]       = df["Release Date Parsed"].dt.year
            df["Release_Month"]      = df["Release Date Parsed"].dt.month
            df["Release_Day"]        = df["Release Date Parsed"].dt.day
            
            df.drop(columns=["Release Date", "Release Date Clean", "Release Date Parsed"],
            inplace=True, errors='ignore')

            # Quitar de N
            N_cols = [c for c in N_cols if c != 'Release Date']
            if 'Release_Year' not in N_cols:
                N_cols.append('Release_Year')
            if 'Release_Month' not in N_cols:
                N_cols.append('Release_Month')
            if 'Release_Day' not in N_cols:
                N_cols.append('Release_Day')

            print("Tamanio: ", df.shape)
            print("Total de columnas: ", len(df.columns))
            print("Columnas numericas usadas: ", N_cols)
            print("Columnas del df: ", df.columns.tolist())
            df[N_cols] = df[N_cols].fillna(0)
            df_NumericCols = df[N_cols]
        ...
        # [Text] splitting
        if TEXT: 
            X_train_text = X[train_idx] # X = text feature
        ...
        X_train_cat = CAT_EMB[train_idx] # X = text feature
        X_test_cat  = CAT_EMB[test_idx]
        if embedding_type == 'tfidf': # c
            X_train_cat = vectorizer_cat.fit_transform(X_train_cat).toarray()
            X_test_cat = vectorizer_cat.transform(X_test_cat).toarray()

        # [Numeric] splitting
        X_train_Numeric = df_NumericCols.iloc[train_idx].values
        X_test_Numeric  = df_NumericCols.iloc[test_idx].values

        print("X_train_Numeric: ", X_train_Numeric.shape)
        print("X_train_Numeric: ", X_train_Numeric.shape)

        ## Concatenation of all data: 
        print("Concatenating TF-IDF and numeric features...")
        X_train = np.concatenate([X_train_Numeric, X_train_cat], axis=1)
        X_test  = np.concatenate([ X_test_Numeric, X_test_cat],  axis=1)
        y_train = y.iloc[train_idx]
        y_test  = y.iloc[test_idx]

        print("Shape of X_train after concatenation: ", X_train.shape)
        print("Shape of X_test after concatenation: ", X_test.shape)
        ...
        output_dir = f"outputs_cv{out}/1/{embedding_type}"
        ...
        if MLP_: 
            ...
            print("Comenzando trainning de MLP...")
            device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
            resumen_mlp, resultados_mlp = train_mlp_kfold(
                X_train, X_test, y_train, y_test,
                nets, device, output_dir, embedding_type,
                n_splits=5, n_epochs=_EPHOCS
            )
            # Fusionar al diccionario global
            resultados_globales.setdefault(embedding_type, {})
            resultados_globales[embedding_type].update(resultados_mlp[embedding_type])
            print("\n\nresultados_globales con DL", resultados_globales)
        ...
        resumen = train_models_cv(X_train, X_test, y_train, y_test, dir_=output_dir, embedding_type=embedding_type)
        # resultados_globales[embedding_type] = resumen
        resultados_globales.setdefault(embedding_type, {})
        resultados_globales[embedding_type].update(resumen)
        print("resultados_globales con ML", resultados_globales)
    ...
    for emb_type, models_dict in resultados_globales.items():
        ...
        for modelo, metricas in sorted(models_dict.items(), key=lambda x: x[1]["f1_score_test"], reverse=True):
            print(f"{modelo}: {metricas}")

    # Guardar resultados globales
    output_file = os.path.join(output_dir, "global_metrics.json")
    with open(output_file, "w") as f:
        json.dump(resultados_globales, f, indent=4)
    print(f"Diccionario global guardado en: {output_file}")
