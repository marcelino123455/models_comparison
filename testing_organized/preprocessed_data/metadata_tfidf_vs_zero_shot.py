    ...
    for embedding_type in embb_types:
        if TEXT: 
            if embedding_type == 'lyrics_bert':
                ...
                X, y = elimnate_index(X, y) # Seguiremos eliminando los 13 indices 
                # Para consistencia comparativa

            elif embedding_type == 'gpt':
                npy_path_gpt = f"{path_DATA}/new_gpt/gpt_fussioned/gpt_T_complete.npy"
                print(f"\n{'#' * 50}")
                print(f"Running experiment with {embedding_type.upper()} embeddings")
        ...
        df_NumericCols = None
        if NUMERICCOlS: # !Numeric cols only works correctly with all dataset since yes skdjsk
            ...
            df_NumericCols = df[N_cols] # Numeric metadata
        ...
        # )
        # open teh smae indexes of deberta zero shot to compare


        df_split_indices = pd.read_csv(f"{zero_shot_path}/split_indices_train.csv", nrows=_SAMPLE_SIZE)
        df_split_indices_test = pd.read_csv(f"{zero_shot_path}/split_indices_test.csv", nrows=_SAMPLE_SIZE)
        
        train_idx = df_split_indices["train_index"].values
        test_idx = df_split_indices_test["test_index"].values
        ...
        output_dir = f"{zero_shot_path}{out}/{embedding_type}"
        if SCALED:
            scaler = MinMaxScaler(feature_range=(0, 1))
        ...
        if MLP_: 
            ...
            n_params = []
            f1_scores = []
            best_f1_score_all = 0.0
            best_predictions_mlp = None
            ...
            for name, capas in nets.items():
                ...
                for i in range(veces):
                    ...
                    if best_f1_score > best_f1_score_all:
                        best_f1_score_all =best_f1_score 
                        best_predictions_mlp = best_pred
                ...
                resumencito["f1_score_avg"] = f1_score_result/veces 
                resumencito["f1_score"] = best_f1_score_all 
        ...
        df_test_predictions = pd.DataFrame({
            "test_index": test_idx,
            "predicted_label": best_predictions_mlp
        })
        output_predictions_path = f"{output_dir}/test_predictions.csv"
        os.makedirs(output_dir, exist_ok=True)
        df_test_predictions.to_csv(output_predictions_path, index=False)
        print("Saved on:", output_predictions_path)
